...
function HomeScreen({ navigation }) {
  ...
  const renderEmoji = (emoji, index) => (
    <TouchableOpacity key={index} onPress={() => setMoods({ ...moods, [selectedDate]: emoji })}>
      <Text style={[styles.emoji, moods[selectedDate] === emoji ? styles.selectedEmoji : null]}>{emoji}</Text>
    </TouchableOpacity>
  );
          ...
          {[9, 10, 11, 12, 13, 14, 15].map((day) => (
            <View key={day} style={styles.dayContainer}>
              <TouchableOpacity onPress={() => setSelectedDate(day)}>
                <Text style={[styles.dayNumber, selectedDate === day ? styles.selectedDay : null]}>{day}</Text>
              </TouchableOpacity>
  ...
  return (
    <Tab.Navigator initialRouteName="Home" screenOptions={{ headerShown: false }}>
      ...
      <Tab.Screen
        ...
        options={{
          title: "마이",
          tabBarIcon: ({ color, size }) => <AntDesign name="solution1" size={size} color={color} />,
        }}
      />
    </Tab.Navigator>
  );
